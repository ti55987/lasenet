...
prAlphaPos = @(x) betapdf(x, 8, 3);
prAlphaNeg = @(x) unifpdf(x, 0.5, 1);
prBeta = @(x) unifpdf(x, 1e-4, 1); %betapdf(x, 5, 5); %
prStickiness = @(x) unifpdf(x, 0, 0.5); 
pdfs = {prAlphaPos, prAlphaNeg, prBeta, prStickiness};

pMin = [1e-4 0.5 1e-4 0]; % Minimum values for model parameters
pMax = [1 1 1 0.5]; % Maximum values for model parameters
...
for i = 1:numel(betaPriors)
    ...
    disp(prefix);
    allAgentData = loaddata(numAgents, numTrials, prefix, numTrials);
    % start fitting with MAP
    fitparams = fitmodel(numAgents, allAgentData, pMin, pMax, pdfs);
    
    allParamas = vertcat(fitparams{:});
    allParamTable = array2table(allParamas, 'VariableNames', { 'alpha', 'nalpha', 'beta', 'stickiness'});
    writetable(allParamTable, sprintf('results/map_%da_%dt_parameters.csv', numAgents, numTrials), 'delimiter',',');
end


function fittedParams = fitmodel(numAgents, allAgentData, pMin, pMax, priors)
    ...
    parfor i = 1:numAgents
        ...
        myfitfun = @(b) neglogposterior(b, D, priors);
...
function nlp = neglogposterior(theta, D, priors)
    posterior = llhRL2apc(par(1),par(2),par(3),par(4),D);
    for i = 1:length(priors)
       posterior = posterior + log(priors{i}(theta(i)));
    end
    nlp = -posterior;
end
